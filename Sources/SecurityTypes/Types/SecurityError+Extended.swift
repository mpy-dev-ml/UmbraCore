...
public extension UmbraErrors.Security.Core {
    ...
    static func keyGenerationFailed(error: Error?) -> UmbraErrors.Security.Core {
        if let error {
    ...
    static func bookmarkResolutionFailed(error: Error) -> UmbraErrors.Security.Core {
    ...
    static func accessDenied(path: String, error: Error) -> UmbraErrors.Security.Core {
