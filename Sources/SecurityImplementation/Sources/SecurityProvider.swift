...
public final class SecurityProvider: SecurityProviderProtocol {
  ...
  public func performSecureOperation(
    ...
    switch operation {
          ...
          switch keyResult {
            case let .success(key):
              return await cryptoService.encryptSymmetric(
                data: config.inputData ?? SecureBytes(bytes: []),
                key: key,
                config: config
              )
            case .failure:
      ...
      case .hashing:
        return await cryptoService.hash(data: config.inputData ?? SecureBytes(bytes: []), config: config)
  ...
  /**
   ...
   - "dGVzdA==" (="test")
   */
  private func base64StringToData(_: String) -> [UInt8]? {
