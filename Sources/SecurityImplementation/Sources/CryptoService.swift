...
public final class CryptoService: CryptoServiceProtocol, Sendable {
  ...
  public func encrypt(data: SecureBytes, config: SecurityConfigDTO) async -> SecurityResultDTO {
    do {
      ...
      return SecurityResultDTO(
        success: false,
        error: .encryptionFailed(reason: "Encryption failed: \(error.localizedDescription)")
      )
    }
  }
    ...
    do {
      ...
      if let providedIv = config.initializationVector {
        ...
        guard data.count > 12 else {
          return SecurityResultDTO(
            success: false,
            error: .invalidInput(reason: "Encrypted data too short")
          )
        }
      ...
      return SecurityResultDTO(
        success: false,
        error: .decryptionFailed(reason: "Decryption failed: \(error.localizedDescription)")
      )
    }
  }
  ...
  ///
  /// This function expects the input data to be in the format: [IV (12 bytes)][Encrypted data with
  /// authentication tag]
  ...
  public nonisolated func verify(
    data: SecureBytes,
  ...
  public nonisolated func generateMAC(
  ...
  public nonisolated func verifyMAC(
    _ mac: SecureBytes,
    for data: SecureBytes,
    using key: SecureBytes
  ) async -> Result<Bool, SecurityError> {
   ...
   */
  public func generateAsymmetricKeyPair() async -> Result<(
    ...
    // Return the key pair
    return .success((publicKey: SecureBytes(bytes: publicKeyBytes), privateKey: privateKey))
  }
  ...
  private func encryptKeyWithPseudoRSA(_ key: SecureBytes, publicKey: SecureBytes) -> SecureBytes {
    ...
    guard !hmacBytes.isEmpty else {
      return SecureBytes(bytes: resultBytes) // Return original key if HMAC failed
    }
  ...
  private func decryptKeyWithPseudoRSA(
    ...
    // Ensure the HMAC has content
    guard !hmacBytes.isEmpty else {
      return SecureBytes(bytes: resultBytes) // Return encrypted key if HMAC failed
    }
