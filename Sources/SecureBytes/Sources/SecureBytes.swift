// SecureBytes.swift - Foundation-free implementation
...
public struct SecureBytes: Sendable, Equatable, Hashable {
    ...
    public init(repeating: UInt8, count: Int) {
        self.storage = [UInt8](repeating: repeating, count: count)
    }
    ...
    public subscript(bounds: Range<Int>) -> SecureBytes {
        get {
            precondition(bounds.lowerBound >= 0 && bounds.upperBound <= storage.count, "Range out of bounds")
    ...
    public func split(at position: Int) throws -> (SecureBytes, SecureBytes) {
        ...
        let secondPart = self[position..<self.count]
    ...
    public func hexString() -> String {
        return storage.map { byteToHexString($0) }.joined()
    }
