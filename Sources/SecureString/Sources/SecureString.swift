...
public struct SecureString: @unchecked Sendable {
    ...
    public init(_ string: String) {
        ...
        // Store the bytes
        self._storage = SecureString.scramble(bytes: bytes)
        self._lock = Lock()
...
extension SecureString: CustomDebugStringConvertible {
    public var debugDescription: String {
        return "<SecureString: length=\(length), content=REDACTED>"
    }
}
...
final class Lock {
    @usableFromInline
    internal var _mutex = UnsafeMutablePointer<pthread_mutex_t>.allocate(capacity: 1)
