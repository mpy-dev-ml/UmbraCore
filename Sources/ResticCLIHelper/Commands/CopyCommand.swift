...
public final class CopyCommand: ResticCommand, @unchecked Sendable {
    ...
    public func validate() throws {
        ...
        if options.validateCredentials && options.password.isEmpty {
            throw ResticError.missingParameter("Source password must not be empty when validation is enabled")
        }
        if validateTargetCredentials && targetPassword.isEmpty {
            throw ResticError.missingParameter("Target password must not be empty when validation is enabled")
        }
