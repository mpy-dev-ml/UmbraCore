...
// Setup utilities for error logging system
//
...
extension ErrorLogger {
    ...
    public func setupSourceFilter(
        ...
        return configure { config in
            ...
            let sourceFilter: (Error) -> Bool = { error in
                guard let umbraError = error as? ErrorHandlingInterfaces.UmbraError,
                      let source = umbraError.source else {
                    return false
                }
    ...
    public func setupSystemInfoLogging() -> ErrorLogger {
        return configure { config in
            ...
            let systemInfoFilter: (Error) -> Bool = { error in
