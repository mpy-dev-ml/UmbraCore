  ...
  public enum Protocols: Error, UmbraError, StandardErrorCapabilities {
    ...
    public var code: String {
      ...
      case .messageEncodingFailed:
        return "message_encoding_failed"
      case .messageDecodingFailed:
        return "message_decoding_failed"
    ...
    public func with(context: ErrorHandlingInterfaces.ErrorContext) -> Self {
      ...
      case let .messageEncodingFailed(protocolName, reason):
        return .messageEncodingFailed(protocolName: protocolName, reason: reason)
      case let .messageDecodingFailed(protocolName, reason):
        return .messageDecodingFailed(protocolName: protocolName, reason: reason)
      case let .unsupportedMessageType(type, protocolName, supportedVersion):
        return .unsupportedMessageType(type: type, protocolName: protocolName, supportedVersion: supportedVersion)
      case let .securityVerificationFailed(protocolName, reason):
        return .securityVerificationFailed(protocolName: protocolName, reason: reason)
      case let .authenticationFailed(protocolName, reason):
        return .authenticationFailed(protocolName: protocolName, reason: reason)
      case let .entitlementMissing(protocolName, entitlement):
        return .entitlementMissing(protocolName: protocolName, entitlement: entitlement)
      case let .internalError(message):
...
extension UmbraErrors.XPC.Protocols {
  ...
  public static func incompatibleVersion(
    version: String,
    file: String = #file,
    line: Int = #line,
    function: String = #function
