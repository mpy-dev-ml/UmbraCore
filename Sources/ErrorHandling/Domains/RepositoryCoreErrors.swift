  ...
  public enum Core: Error, UmbraError, StandardErrorCapabilities, ResourceErrors {
    ...
    public var errorDescription: String {
      ...
      case let .repositoryNotFound(resource):
        return "Repository not found: \(resource)"
      case let .repositoryOpenFailed(reason):
        return "Failed to open repository: \(reason)"
      case let .repositoryCorrupt(reason):
        return "Repository is corrupt: \(reason)"
      case let .repositoryLocked(owner):
        if let owner = owner {
          return "Repository is locked by: \(owner)"
        } else {
          return "Repository is locked by another process"
        }
      case let .invalidState(state, expectedState):
        return "Repository is in invalid state: current '\(state)', expected '\(expectedState)'"
      case let .permissionDenied(operation, reason):
        return "Permission denied for operation '\(operation)': \(reason)"
      case let .objectNotFound(objectId, objectType):
        if let type = objectType {
          return "Object not found: \(type) with ID \(objectId)"
        } else {
          return "Object not found: \(objectId)"
        }
      case let .objectAlreadyExists(objectId, objectType):
        if let type = objectType {
          return "Object already exists: \(type) with ID \(objectId)"
        } else {
          return "Object already exists: \(objectId)"
        }
      case let .objectCorrupt(objectId, reason):
        return "Object is corrupt (ID: \(objectId)): \(reason)"
      case let .invalidObjectType(providedType, expectedType):
        return "Invalid object type: provided '\(providedType)', expected '\(expectedType)'"
      case let .invalidObjectData(objectId, reason):
        return "Invalid object data (ID: \(objectId)): \(reason)"
      case let .saveFailed(objectId, reason):
        return "Failed to save object (ID: \(objectId)): \(reason)"
      case let .deleteFailed(objectId, reason):
        return "Failed to delete object (ID: \(objectId)): \(reason)"
      case let .updateFailed(objectId, reason):
        return "Failed to update object (ID: \(objectId)): \(reason)"
      case let .timeout(operation, timeoutMs):
        return "Operation '\(operation)' timed out after \(timeoutMs)ms"
      case let .internalError(reason):
        return "Internal repository error: \(reason)"
      }
    }
    ...
    public func with(context: ErrorHandlingInterfaces.ErrorContext) -> Self {
      ...
      switch self {
      case let .repositoryNotFound(resource):
        return .repositoryNotFound(resource: resource)
      case let .repositoryOpenFailed(reason):
        return .repositoryOpenFailed(reason: reason)
      case let .repositoryCorrupt(reason):
        return .repositoryCorrupt(reason: reason)
      case let .repositoryLocked(owner):
        return .repositoryLocked(owner: owner)
      case let .invalidState(state, expectedState):
        return .invalidState(state: state, expectedState: expectedState)
      case let .permissionDenied(operation, reason):
        return .permissionDenied(operation: operation, reason: reason)
      case let .objectNotFound(objectId, objectType):
        return .objectNotFound(objectId: objectId, objectType: objectType)
      case let .objectAlreadyExists(objectId, objectType):
        return .objectAlreadyExists(objectId: objectId, objectType: objectType)
      case let .objectCorrupt(objectId, reason):
        return .objectCorrupt(objectId: objectId, reason: reason)
      case let .invalidObjectType(providedType, expectedType):
        return .invalidObjectType(providedType: providedType, expectedType: expectedType)
      case let .invalidObjectData(objectId, reason):
        return .invalidObjectData(objectId: objectId, reason: reason)
      case let .saveFailed(objectId, reason):
        return .saveFailed(objectId: objectId, reason: reason)
      case let .deleteFailed(objectId, reason):
        return .deleteFailed(objectId: objectId, reason: reason)
      case let .updateFailed(objectId, reason):
        return .updateFailed(objectId: objectId, reason: reason)
      case let .timeout(operation, timeoutMs):
...
extension UmbraErrors.Repository.Core {
  ...
  public static func objectNotFound(
    id: String,
    type: String? = nil,
    file: String = #file,
    line: Int = #line,
