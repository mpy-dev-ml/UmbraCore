...
public actor CredentialManager {
  ...
  public func store(credential: Data, withIdentifier identifier: String) async throws {
    ...
    let encryptResult = await encrypt(data: credentialBytes, using: keyBytes, iv: SecureBytes(data: iv))
  ...
  private func encrypt(data: SecureBytes, using key: SecureBytes, iv: SecureBytes) async -> Result<SecureBytes, XPCSecurityError> {
    // Using the direct encrypt method from ModernCryptoXPCServiceProtocol
    return await xpcService.encrypt(data, key: key)
  }
  
  // Helper method to decrypt data using the XPC service
  private func decrypt(data: SecureBytes, using key: SecureBytes, iv: SecureBytes) async -> Result<SecureBytes, XPCSecurityError> {
    // Using the direct decrypt method from ModernCryptoXPCServiceProtocol
    return await xpcService.decrypt(data, key: key)
  }
}
