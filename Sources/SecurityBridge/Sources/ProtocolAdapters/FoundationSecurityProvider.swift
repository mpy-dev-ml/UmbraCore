// FoundationSecurityProvider.swift
// SecurityBridge
//
...
@objc public protocol FoundationSecurityProviderObjC: NSObjectProtocol, Sendable {
    ...
    @objc func performOperation(operation: String, options: [String: Any]) async -> FoundationOperationResult
}

/// Protocol for Foundation-based security implementations
@objc public protocol SecurityProviderFoundationImpl: FoundationSecurityProviderObjC {
    ...
    @objc func createBookmark(for url: URL) async throws -> Foundation.Data
    
    /// Resolve a bookmark to a URL
    /// - Parameter bookmarkData: Bookmark data to resolve
    /// - Returns: URL and stale flag tuple
    @objc func resolveBookmark(_ bookmarkData: Foundation.Data) async throws -> (url: URL, isStale: Bool)
    
    /// Validate if a bookmark is still valid
    /// - Parameter bookmarkData: Bookmark data to validate
    /// - Returns: True if valid, false otherwise
    @objc func validateBookmark(_ bookmarkData: Foundation.Data) async throws -> Bool
}

/// Result type for Foundation security operations
@objc public protocol FoundationOperationResult: NSObjectProtocol {
...
@objc public final class FoundationOperationResultImpl: NSObject, FoundationOperationResult {
    ...
    public static func success(_ data: Data?) -> FoundationOperationResultImpl {
        return FoundationOperationResultImpl(.success(data))
    }
...
extension SecurityProviderFoundationImpl {
    public func performOperation(operation: String, options: [String: Any]) async -> FoundationOperationResult {
        do {
            switch operation {
            case "encrypt":
                guard let data = options["data"] as? Data, let key = options["key"] as? Data else {
                    return FoundationOperationResultImpl.failure(NSError(domain: "SecurityProvider", code: 100, userInfo: [NSLocalizedDescriptionKey: "Invalid parameters for encrypt operation"]))
                }
            ...
            case "decrypt":
                guard let data = options["data"] as? Data, let key = options["key"] as? Data else {
                    return FoundationOperationResultImpl.failure(NSError(domain: "SecurityProvider", code: 101, userInfo: [NSLocalizedDescriptionKey: "Invalid parameters for decrypt operation"]))
                }
            ...
            case "generateRandomData":
                guard let length = options["length"] as? Int else {
                    return FoundationOperationResultImpl.failure(NSError(domain: "SecurityProvider", code: 102, userInfo: [NSLocalizedDescriptionKey: "Invalid parameters for generateRandomData operation"]))
                }
                let result = try await generateDataKey(length: length)
                return FoundationOperationResultImpl.success(result)
            ...
            case "hash":
                guard let data = options["data"] as? Data else {
                    return FoundationOperationResultImpl.failure(NSError(domain: "SecurityProvider", code: 103, userInfo: [NSLocalizedDescriptionKey: "Invalid parameters for hash operation"]))
                }
            ...
            case "createBookmark":
                guard let urlString = options["url"] as? String, let url = URL(string: urlString) else {
                    return FoundationOperationResultImpl.failure(NSError(domain: "SecurityProvider", code: 104, userInfo: [NSLocalizedDescriptionKey: "Invalid parameters for createBookmark operation"]))
                }
            ...
            case "resolveBookmark":
                guard let bookmarkData = options["bookmarkData"] as? Data else {
                    return FoundationOperationResultImpl.failure(NSError(domain: "SecurityProvider", code: 105, userInfo: [NSLocalizedDescriptionKey: "Invalid parameters for resolveBookmark operation"]))
                }
            ...
            case "validateBookmark":
                guard let bookmarkData = options["bookmarkData"] as? Data else {
                    return FoundationOperationResultImpl.failure(NSError(domain: "SecurityProvider", code: 106, userInfo: [NSLocalizedDescriptionKey: "Invalid parameters for validateBookmark operation"]))
                }
            ...
            default:
                return FoundationOperationResultImpl.failure(NSError(domain: "SecurityProvider", code: 107, userInfo: [NSLocalizedDescriptionKey: "Unsupported operation: \(operation)"]))
            }
        } catch {
            return FoundationOperationResultImpl.failure(error)
        }
