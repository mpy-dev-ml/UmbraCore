...
public final class KeyManagementXPCAdapter: NSObject, BaseXPCAdapter {
    ...
    private func executeKeyManagementSelector<T>(_ selector: String, withArguments arguments: [Any] = []) async -> T? {
        return await executeXPCSelector(selector, withArguments: arguments)
    }
}
        ...
        await withCheckedContinuation { continuation in
            Task {
                ...
                if let result = result as? [String: String] {
                    ...
                    continuation.resume(returning: .failure(UmbraErrors.Security.XPC.invalidFormat(reason: "Invalid metadata format")))
                }
            }
        }
    }
