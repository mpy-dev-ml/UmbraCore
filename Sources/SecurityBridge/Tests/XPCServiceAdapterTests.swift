...
class ErrorConnection: NSXPCConnection {
    ...
    override var remoteObjectProxy: Any {
        ...
        NSError(domain: "com.umbracore.test", code: -1, userInfo: [NSLocalizedDescriptionKey: "Test connection error"])
    }
...
final class XPCServiceAdapterTests: XCTestCase {
    ...
    func testCreateCryptoService() async {
        ...
        XCTAssertTrue(cryptoService is CryptoServiceProtocol, "Crypto service should implement CryptoServiceProtocol")
    }
    ...
    func testCreateKeyManagement() async {
        ...
        // Avoid direct type casting which might cause issues if implementation changes
        XCTAssertTrue(keyManagement is KeyManagementProtocol, "Key management should implement KeyManagementProtocol")
    }
    ...
    func testKeyManagementListKeys() async {
        ...
        if case .success(let keys) = result {
    ...
    func testKeyManagementRetrieveKeyFailure() async {
        ...
        if case .failure(let error) = result {
            XCTAssertEqual(error.localizedDescription.contains("not found") || error.localizedDescription.contains("Key not found"), true)
        } else {
    ...
    func testKeyManagementStoreKeyFailure() async {
        ...
        // Assert
        if case .failure(let error) = result {
            XCTAssertTrue(error.localizedDescription.contains("Failed") || error.localizedDescription.contains("failed"))
        } else {
    ...
    func testKeyManagementDeleteKeyFailure() async {
        ...
        // Assert
        if case .failure(let error) = result {
            XCTAssertTrue(error.localizedDescription.contains("Cannot") || error.localizedDescription.contains("cannot"))
        } else {
