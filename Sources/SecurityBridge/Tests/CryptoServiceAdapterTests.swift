// CryptoServiceAdapterTests.swift
// SecurityBridge
//
...
private final class MockCryptoServiceAdapter: FoundationCryptoService, @unchecked Sendable {
        ...
        return await withCheckedContinuation { continuation in
            mockXPCService.encrypt(data: data, key: key) { data, error in
                if let error = error {
                    ...
                    continuation.resume(returning: .failure(NSError(domain: "com.umbracore.mock", code: 500, userInfo: [NSLocalizedDescriptionKey: "Unknown error"])))
                }
            }
        }
    }
    ...
    func hash(data: Data) async -> Result<Data, Error> {
        return await withCheckedContinuation { continuation in
            ...
            mockXPCService.hashDataXPC(data: data, algorithm: "SHA-256", optionsJson: "{}") { data, code, errorMessage in
                if let code = code, code.intValue != 0 {
                    let error = NSError(domain: "com.umbracore.security", code: code.intValue, userInfo: [NSLocalizedDescriptionKey: errorMessage ?? "Hash operation failed"])
                    continuation.resume(returning: .failure(error))
                } else if let data = data {
                    continuation.resume(returning: .success(data))
                } else {
                    let error = NSError(domain: "com.umbracore.security", code: 500, userInfo: [NSLocalizedDescriptionKey: "Unknown error"])
                    continuation.resume(returning: .failure(error))
                }
            }
        }
    ...
    func encryptSymmetric(
        ...
        return await withCheckedContinuation { continuation in
            ...
            mockXPCService.encryptSymmetricXPC(
                ...
                if let code = code, code.intValue != 0 {
                    continuation.resume(returning: FoundationSecurityResult(errorCode: code.intValue, errorMessage: errorMessage ?? "Encryption failed"))
                } else if let data = data {
                    continuation.resume(returning: FoundationSecurityResult(data: data))
                } else {
                    continuation.resume(returning: FoundationSecurityResult(errorCode: 500, errorMessage: "Unknown error"))
                }
            }
        }
    }
    ...
    func decryptSymmetric(
        ...
        return await withCheckedContinuation { continuation in
            ...
            mockXPCService.decryptSymmetricXPC(
                ...
                if let code = code, code.intValue != 0 {
                    continuation.resume(returning: FoundationSecurityResult(errorCode: code.intValue, errorMessage: errorMessage ?? "Decryption failed"))
                } else if let data = data {
                    continuation.resume(returning: FoundationSecurityResult(data: data))
                } else {
                    continuation.resume(returning: FoundationSecurityResult(errorCode: 500, errorMessage: "Unknown error"))
    ...
    func encryptAsymmetric(
        ...
        return await withCheckedContinuation { continuation in
            ...
            mockXPCService.encryptAsymmetricXPC(
                ...
                if let code = code, code.intValue != 0 {
                    continuation.resume(returning: FoundationSecurityResult(errorCode: code.intValue, errorMessage: errorMessage ?? "Asymmetric encryption failed"))
                } else if let data = data {
                    continuation.resume(returning: FoundationSecurityResult(data: data))
                } else {
                    continuation.resume(returning: FoundationSecurityResult(errorCode: 500, errorMessage: "Unknown error"))
    ...
    func decryptAsymmetric(
        ...
        return await withCheckedContinuation { continuation in
            ...
            mockXPCService.decryptAsymmetricXPC(
                ...
                if let code = code, code.intValue != 0 {
                    continuation.resume(returning: FoundationSecurityResult(errorCode: code.intValue, errorMessage: errorMessage ?? "Asymmetric decryption failed"))
                } else if let data = data {
                    continuation.resume(returning: FoundationSecurityResult(data: data))
                } else {
                    continuation.resume(returning: FoundationSecurityResult(errorCode: 500, errorMessage: "Unknown error"))
    ...
    func hash(
        ...
        return await withCheckedContinuation { continuation in
            ...
            mockXPCService.hashDataXPC(
                ...
                if let code = code, code.intValue != 0 {
                    continuation.resume(returning: FoundationSecurityResult(errorCode: code.intValue, errorMessage: errorMessage ?? "Hashing failed"))
                } else if let data = data {
                    continuation.resume(returning: FoundationSecurityResult(data: data))
                } else {
                    continuation.resume(returning: FoundationSecurityResult(errorCode: 500, errorMessage: "Unknown error"))
...
final class CryptoServiceAdapterTests: XCTestCase {
    ...
    func testEncrypt() async throws {
        ...
        if case .success(let encryptedData) = result {
    ...
    func testDecrypt() async throws {
        ...
        // Assert
        XCTAssertTrue(result.isSuccess)
        if case .success(let decryptedData) = result {
