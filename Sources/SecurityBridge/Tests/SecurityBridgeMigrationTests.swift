...
final class SecurityBridgeMigrationTests: XCTestCase {
  ...
  func testSecurityErrorMapping() {
    ...
    XCTAssertTrue(
      (mappedBackError as? SecurityError)?.description
        .starts(with: "Internal") ?? false
    )
  }
  ...
  func testSecurityErrorBridging() {
    ...
    // Check error mapping
    if case let .implementationMissing(message)=bridgeError {
...
private class MockXPCServiceProtocolBase: SecurityInterfacesProtocols.XPCServiceProtocolBase,
@unchecked Sendable {
  ...
  func synchroniseKeys(_: SecurityInterfacesProtocols.BinaryData) async throws {
    // No-op for mock
  }
...
@objc
private class MockXPCServiceProtocolFoundationBridge: NSObject,
XPCServiceProtocolFoundationBridge {
  ...
  func synchroniseKeysFoundation(_: NSData, withReply reply: @escaping @Sendable (Error?) -> Void) {
    reply(nil)
  }

  func encryptFoundation(
    data: NSData,
    withReply reply: @escaping @Sendable (NSData?, Error?) -> Void
  ) {
    // Simple "encryption" for test
...
private class MockSecurityProviderBridge: SecurityBridgeProtocolAdapters.SecurityProviderBridge,
@unchecked Sendable {
  static var protocolIdentifier: String="mock.provider.bridge"

  func encrypt(_ data: DataBridge, key _: DataBridge) async throws -> DataBridge {
    // Simple "encryption" for test
    var bytes=data.bytes
    for i in 0..<bytes.count {
      bytes[i]=bytes[i] ^ 0xFF
  ...
  func decrypt(_ data: DataBridge, key _: DataBridge) async throws -> DataBridge {
  ...
  func resolveBookmark(_ bookmarkData: DataBridge) async throws
  -> (urlString: String, isStale: Bool) {
  ...
  func validateBookmark(_: DataBridge) async throws -> Bool {
