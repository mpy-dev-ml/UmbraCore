...
public final class MockXPCService: XPCProtocolsCore.XPCServiceProtocolStandard {
    ...
    public func storeSecureData(key _: String, data _: Data) async -> Result<Bool, XPCProtocolsCore.SecurityError> {
        .success(true)
    }
    ...
    public func deleteSecureData(key _: String) async -> Result<Bool, XPCProtocolsCore.SecurityError> {
        .success(true)
    }

    public func encryptSecureData(_ data: Data, keyIdentifier _: String) async -> Result<Data, XPCProtocolsCore.SecurityError> {
        // Just return the data unchanged for testing
        .success(data)
    }

    public func decryptSecureData(_ data: Data, keyIdentifier _: String) async -> Result<Data, XPCProtocolsCore.SecurityError> {
        // Just return the data unchanged for testing
        .success(data)
    }

    public func sign(_: Data, keyIdentifier _: String) async -> Result<Data, XPCProtocolsCore.SecurityError> {
        // Return some mock signature
        let mockSignature = Data([0x01, 0x02, 0x03, 0x04])
        return .success(mockSignature)
    }

    public func verify(signature _: Data, for _: Data, keyIdentifier _: String) async -> Result<Bool, XPCProtocolsCore.SecurityError> {
        .success(true)
    }
    ...
    public func deleteKey(_: String) async -> Result<Bool, XPCProtocolsCore.SecurityError> {
        .success(true)
    }
}
