...
/// Type alias for SecurityProtocolsCore's error type - We use the same prefix as in SPCorePackage.swift
...
public func mapToSecurityProtocolsCore(_ error: NSError) -> SPCSecurityError {
    ...
    case 102:
        return .keyGenerationFailed(
            reason: error.userInfo[NSLocalizedDescriptionKey] as? String ?? "Unknown key generation failure"
        )
    case 103:
        return .invalidKey
    ...
    case 105:
        return .randomGenerationFailed(
            reason: error.userInfo[NSLocalizedDescriptionKey] as? String ?? "Unknown random generation failure"
        )

    // Input and Data Errors
    case 106:
        return .invalidInput(
            reason: error.userInfo[NSLocalizedDescriptionKey] as? String ?? "Unknown input validation failure"
        )
    case 107:
    ...
    case 108:
        return .timeout
    case 109:
        let description = error.userInfo[NSLocalizedDescriptionKey] as? String ?? "Unknown service error"
        return .serviceError(
            code: error.code,
            reason: description
        )
    ...
    // General Errors
    case 110:
        return .internalError(error.userInfo[NSLocalizedDescriptionKey] as? String ?? "Unknown reason")
    case 111:
