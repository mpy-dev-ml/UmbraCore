...
final class ResourceLocatorTests: XCTestCase {
    ...
    func testMapToCoreErrors() {
        ...
        for error in errors {
            ...
            switch error {
            case .invalidPath:
                if let resourceError = mappedError as? CEResourceError,
            ...
            case .accessDenied:
                if let securityError = mappedError as? CESecurityError,
    ...
    func testMapFromCoreErrors() {
        ...
        for coreError in errors {
            ...
            case .resourceNotFound:
                XCTAssertEqual(mappedError as? ResourceLocatorError, ResourceLocatorError.resourceNotFound)
                
            case .operationFailed:
                XCTAssertEqual(mappedError as? ResourceLocatorError, ResourceLocatorError.generalError("Operation failed"))
                
            case .acquisitionFailed:
                XCTAssertEqual(mappedError as? SecureBytesError, SecureBytesError.allocationFailed)
                
            case .poolExhausted:
                if let resourceError = mappedError as? ResourceLocatorError,
                   case .generalError(let message) = resourceError {
