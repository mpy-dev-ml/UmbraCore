...
final class CoreErrorsMappingTests: XCTestCase {
    ...
    func testBidirectionalMapping() {
        ...
        // Verify the mapped error
        XCTAssertTrue(umbralError is ResourceLocatorError, "Expected ResourceLocatorError but got \(type(of: umbralError))")
        
        // Map back to CoreErrors
        let remappedError = mapToCoreErrors(umbralError)
        
        // Verify the re-mapped error
        XCTAssertTrue(remappedError is CEResourceError, "Expected CEResourceError but got \(type(of: remappedError))")
    }
    ...
    func testNamespaceResolution() {
        ...
        XCTAssertNotEqual(String(describing: type(of: coreError)), 
