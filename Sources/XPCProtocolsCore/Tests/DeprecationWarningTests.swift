...
class DeprecationWarningTests: XCTestCase {
    ...
    func testDeprecatedProtocolStillFunctional() async throws {
        ...
        let result = try await legacyService.encrypt(data: SecurityInterfacesProtocols.BinaryData([1, 2, 3, 4]))
...
private class LegacyService: SecurityInterfaces.XPCServiceProtocol {
    func encrypt(data: SecurityInterfacesProtocols.BinaryData) async throws -> SecurityInterfacesProtocols.BinaryData {
        return data // Mock implementation
    }
    
    func decrypt(data: SecurityInterfacesProtocols.BinaryData) async throws -> SecurityInterfacesProtocols.BinaryData {
        return data // Mock implementation
    }
}
...
extension LegacyService: CryptoXPCServiceProtocol {
    func generateKey(bits: Int) async throws -> Data {
        return Data(count: 4) // Mock implementation
    }
...
private class MigratedService: XPCServiceProtocolStandard {
    func generateRandomData(length: Int) async throws -> UmbraCoreTypes.SecureBytes {
        return UmbraCoreTypes.SecureBytes(bytes: Array(repeating: 0, count: length))
    }
    
    func encryptData(_ data: UmbraCoreTypes.SecureBytes, keyIdentifier: String?) async throws -> UmbraCoreTypes.SecureBytes {
        return data // Mock implementation
    }
    
    func decryptData(_ data: UmbraCoreTypes.SecureBytes, keyIdentifier: String?) async throws -> UmbraCoreTypes.SecureBytes {
        return data // Mock implementation
    }
    ...
    func signData(_ data: UmbraCoreTypes.SecureBytes, keyIdentifier: String) async throws -> UmbraCoreTypes.SecureBytes {
        return UmbraCoreTypes.SecureBytes(bytes: Array(repeating: 0, count: 64))
    }
    
    func verifySignature(_ signature: UmbraCoreTypes.SecureBytes, for data: UmbraCoreTypes.SecureBytes, keyIdentifier: String) async throws -> Bool {
        return true
    }
}
...
private class ModernService: XPCServiceProtocolComplete {
    ...
    func synchronizeKeys(_ secureBytes: UmbraCoreTypes.SecureBytes) async -> Result<Void, XPCSecurityError> {
        return .success(())
    }
    ...
    func generateKey() async -> Result<UmbraCoreTypes.SecureBytes, XPCSecurityError> {
        return .success(UmbraCoreTypes.SecureBytes(bytes: Array(repeating: 0, count: 32)))
    }
    
    func hash(data: UmbraCoreTypes.SecureBytes) async -> Result<UmbraCoreTypes.SecureBytes, XPCSecurityError> {
        return .success(data)
    }
