...
public class KeyExchangeDTOAdapter: KeyExchangeDTOProtocol {
    ...
    public init(service: any (KeyManagementDTOProtocol & XPCServiceProtocolDTO)) {
        self.keyManagementService = service
    ...
    public func calculateSharedSecretWithDTO(
        ...
        let result: OperationResultDTO<SecureBytes>
        if let keyService = keyManagementService as? XPCServiceProtocolDTO {
            result = await keyService.encryptWithDTO(
                data: privateKey,
                config: secretConfig
            )
        } else {
        ...
        let _ = await keyManagementService.deleteKeyWithDTO(keyIdentifier: publicKeyId)
        let _ = await keyManagementService.deleteKeyWithDTO(keyIdentifier: privateKeyId)
...
extension XPCServiceProtocolDTO where Self: KeyManagementDTOProtocol {
    /// Create a key exchange adapter for this service
    /// - Returns: Key exchange adapter
    public func keyExchangeAdapter() -> KeyExchangeDTOProtocol {
