...
public extension SecureBytes {
    ...
    static func fromASCIIString(_ string: String) -> SecureBytes? {
        var bytes: [UInt8] = []
        for char in string {
            guard let scalar = char.unicodeScalars.first,
                  scalar.value <= 127,  // ASCII range
                  let byte = UInt8(exactly: scalar.value) else {
    ...
    func substring(start: Int, length: Int) -> SecureBytes {
