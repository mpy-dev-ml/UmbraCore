// SecurityProviderProtocolAdapter.swift
...
public extension SecurityProviderBridge {
    ...
    func generateKey() async throws -> DataBridge {
        return try await generateKey(length: 32) // Default to 32 bytes (256 bits)
    }

    /// Default implementation to handle the specific length
    func generateKey(length: Int) async throws -> DataBridge {
        return try await generateRandomData(length: length)
    }
}
...
public final class SecurityProviderProtocolAdapter: SecurityInterfacesProtocols.SecurityProviderProtocol {
    ...
    private func mapToProtocolError(_ error: Error) -> SecurityInterfacesProtocols.SecurityProtocolError {
        ...
        if let bridgeError = error as? SecurityBridgeError {
            errorDescription = "Bridge error: \(bridgeError)"
        } else if let secError = error as? SecurityError {
            errorDescription = "Security error: \(secError)"
        } else if let xpcError = error as? SecurityError {
            errorDescription = "XPC error: \(xpcError)"
        } else {
            errorDescription = "Unknown error: \(String(describing: error))"
        }
