...
@globalActor public actor PathAccessTracker {
...
public struct PathURLProvider: URLProvider {
    ...
    public func createBookmark(forPath path: String) async -> Result<Data, UmbraErrors.Security.Protocols> {
        guard let url = URL(string: path) else {
            return .failure(.invalidInput("Invalid URL path: \(path)"))
        }

        do {
            let bookmarkData = try url.bookmarkData(options: .securityScopeAllowOnlyReadAccess,
                                              includingResourceValuesForKeys: nil,
                                              relativeTo: nil)
            return .success(bookmarkData)
        } catch {
            return .failure(.storageOperationFailed("Failed to create bookmark: \(error.localizedDescription)"))
        }
    }
    ...
    public func resolveBookmark(_ bookmarkData: [UInt8]) async
        ...
        do {
            ...
            return .failure(.storageOperationFailed("Failed to resolve bookmark: \(error.localizedDescription)"))
        }
    }
    ...
    public func startAccessing(path: String) async -> Result<Bool, UmbraErrors.Security.Protocols> {
        ...
        do {
            ...
            return .failure(.storageOperationFailed("Error accessing resource: \(error.localizedDescription)"))
        }
    }
    ...
    public func isAccessing(path: String) async -> Bool {
        return await PathAccessTracker.shared.containsPath(path)
    }
    ...
    public func getAccessedPaths() async -> Set<String> {
        return await PathAccessTracker.shared.getAllPaths()
    }
