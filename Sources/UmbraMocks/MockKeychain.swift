...
public final class MockKeychain: @unchecked Sendable, SecureStorageServiceProtocol {
    ...
    public func storeData(
        ...
        storageQueue.async(flags: .barrier) { [self] in
            self.storageDict[identifier] = (data, metadata)
        }
        return .success(())
    }
    ...
    public func retrieveData(
        ...
        storageQueue.sync { [self] in
            if let (data, _) = self.storageDict[identifier] {
                result = .success(data)
            }
        }
    ...
    public func deleteData(
        ...
        storageQueue.async(flags: .barrier) { [self] in
            self.storageDict.removeValue(forKey: identifier)
        }
        return .success(())
    }
    ...
    public func listDataIdentifiers() async -> Result<[String], XPCSecurityError> {
        ...
        storageQueue.sync { [self] in
            keys = Array(self.storageDict.keys)
        }
    ...
    public func getDataMetadata(
        ...
        storageQueue.sync { [self] in
            if let (_, metadata) = self.storageDict[identifier] {
