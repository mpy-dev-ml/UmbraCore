...
public final class ErrorRegistry {
    ...
    public func mapError<T: Error>(_ error: Error, to targetType: T.Type) -> T? {
        ...
        logger.warning("No suitable mapper found for error: \(String(describing: error)) to type \(String(describing: T.self))")
        return nil
    }
...
public extension Error {
    ...
    func mapped(to targetDomain: String) -> Error {
        return ErrorRegistry.shared.mapError(self, to: targetDomain)
    }
    ...
    func mapped<T: Error>(to targetType: T.Type) -> T? {
        return ErrorRegistry.shared.mapError(self, to: targetType)
    }
}
