...
/// Protocol defining the XPC service interface for key management using Objective-C compatible methods
@objc public protocol XPCServiceProtocolDefinition: ObjCBridgingTypesFoundation.XPCServiceProtocolDefinitionBaseFoundation {
    /// Synchronize keys across processes with raw bytes using NSData
    /// - Parameter data: The key data to synchronize
    @objc func synchroniseKeys(_ data: NSData, withReply reply: @escaping @Sendable (Error?) -> Void)

    /// Get the XPC service version
    @objc func getVersion(withReply reply: @escaping @Sendable (NSString?, Error?) -> Void)

    /// Get the host identifier
    @objc func getHostIdentifier(withReply reply: @escaping @Sendable (NSString?, Error?) -> Void)

    /// Register a client application
    @objc func registerClient(clientId: NSString, withReply reply: @escaping @Sendable (Bool, Error?) -> Void)

    /// Deregister a client application
    @objc func deregisterClient(clientId: NSString, withReply reply: @escaping @Sendable (Bool, Error?) -> Void)

    /// Check if a client is registered
    @objc func isClientRegistered(clientId: NSString, withReply reply: @escaping @Sendable (Bool, Error?) -> Void)
}

/// Implementation of XPCServiceProtocolDefinitionBaseFoundation interface
public class XPCServiceProtocolDefinitionImpl: NSObject, ObjCBridgingTypesFoundation.XPCServiceProtocolDefinitionBaseFoundation {
...
extension XPCServiceProtocolDefinitionImpl {
    public func ping(completion: @escaping @Sendable (Bool, Error?) -> Void) {
        ping(withReply: completion)
    }

    public func resetSecurityData(completion: @escaping @Sendable (Error?) -> Void) {
