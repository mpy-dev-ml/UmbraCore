...
public actor ServiceContainer {
  ...
  public func register<T: UmbraService>(_ service: T, dependencies: [String] = []) async throws {
    ...
    guard services[identifier] == nil else {
      throw ServiceError.configurationError("Service with identifier \(identifier) already registered")
    }
    ...
    for serviceId in serviceIds {
      ...
      let initializer: () async throws -> Void = { [weak self] in
        do {
          ...
          throw ServiceError
            .initialisationFailed("Failed to initialize \(serviceId): \(error.localizedDescription)")
        }
      }
  ...
  public func initialiseService(_ identifier: String) async throws {
    ...
    do {
      ...
      throw ServiceError.initialisationFailed("Failed to initialize \(identifier): \(error.localizedDescription)")
    }
  }
  ...
  /// - Throws: ServiceError if circular dependency detected
  private func visit(_ serviceId: String, visited: inout Set<String>, visiting: inout Set<String>, sorted: inout [String]) throws {
    if visited.contains(serviceId) {
      return
    }
    
    if visiting.contains(serviceId) {
      throw ServiceError.dependencyError("Circular dependency detected involving service \(serviceId)")
    }
    ...
    for depId in deps {
      if !services.keys.contains(depId) {
        throw ServiceError.dependencyError("Service \(serviceId) depends on \(depId) which is not registered")
      }
