...
final class UmbraErrorProtocolTests: XCTestCase {
    ...
    func testUmbraErrorBasicRequirements() {
        ...
        XCTAssertEqual(error.errorDescription, "Test error", "Error description should match the provided description")

        // Test description formatting
        XCTAssertEqual(error.description, "[Test:E001] Test error",
    ...
    func testErrorWithSourceLocation() {
        ...
        XCTAssertEqual(errorWithSource.source?.file, "TestFile.swift", "File should match provided file")
        XCTAssertEqual(errorWithSource.source?.function, "testFunction", "Function should match provided function")
        XCTAssertEqual(errorWithSource.source?.line, 42, "Line should match provided line")

        // Verify description format
        XCTAssertTrue(errorWithSource.description.contains("[Test:E001]"),
                      "Error description should contain domain and code")
        XCTAssertTrue(errorWithSource.description.contains("TestFile.swift:42"),
    ...
    func testWithSourceUsingCallerLocation() {
        ...
        // Then
        XCTAssertNotNil(errorWithSource.source, "Source information should be set")
        XCTAssertEqual(errorWithSource.source?.function, #function, "Function should match current function")
        XCTAssertTrue(errorWithSource.source?.file.contains("UmbraErrorProtocolTests.swift") ?? false,
                      "File should match current file")

        // Verify description contains source
        XCTAssertTrue(errorWithSource.description.contains(#function),
    ...
    func testErrorWithContext() {
        ...
        // Then
        XCTAssertEqual(errorWithContext.context.source, "TestSource", "Context source should match provided value")
        XCTAssertEqual(errorWithContext.context.operation, "TestOperation", "Context operation should match provided value")
        XCTAssertEqual(errorWithContext.context.details, "Detailed info", "Context details should match provided value")
    }
    ...
    func testErrorWithUnderlyingError() {
        ...
        XCTAssertEqual((errorWithUnderlying.underlyingError as? NSError)?.domain, "OriginalError",
                       "Underlying error domain should match original error")
        XCTAssertEqual((errorWithUnderlying.underlyingError as? NSError)?.code, 100,
    ...
    func testDomainErrorImplementation() {
        ...
        // Then
        XCTAssertEqual(error.domain, "CustomDomain",
