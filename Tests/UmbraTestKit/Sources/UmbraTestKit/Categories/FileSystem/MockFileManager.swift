...
public final class MockFileManager: Resettable, @unchecked Sendable {
    ...
    public func simulateCreateDirectory(at url: URL, withIntermediateDirectories createIntermediates: Bool = false) throws {
    ...
    public func simulateFileExists(atPath path: String, isDirectory: UnsafeMutablePointer<ObjCBool>? = nil) -> Bool {
    ...
    public func simulateContents(atPath path: String) throws -> Data {
        ...
        for component in components {
            ...
            if component == components.last {
                if item.isDirectory {
                    throw NSError(domain: NSCocoaErrorDomain, code: NSFileReadInvalidFileNameError, userInfo: nil)
                }
    ...
    public func simulateAttributes(ofItemAtPath path: String) throws -> [FileAttributeKey: Any] {
        ...
        for component in components {
            ...
            if component == components.last {
                ...
                attributes[.type] = item.isDirectory ? FileAttributeType.typeDirectory : FileAttributeType.typeRegular
    ...
    public func simulateSetAttributes(_ attributes: [FileAttributeKey: Any], ofItemAtPath path: String) throws {
    ...
    public func simulateURLsForDirectory(at url: URL, includingPropertiesForKeys keys: [URLResourceKey]? = nil, options: FileManager.DirectoryEnumerationOptions = []) throws -> [URL] {
        try checkForSimulatedError(path: url.path)
    ...
    public func simulateCoordinatedReading<T>(from url: URL, options: NSFileCoordinator.ReadingOptions = [], byAccessor: (URL) throws -> T) throws -> T {
    ...
    public func simulateCoordinatedWriting<T>(to url: URL, options: NSFileCoordinator.WritingOptions = [], byAccessor: (URL) throws -> T) throws -> T {
        // For the mock, we'll just call the accessor directly
        // In a real implementation, this would handle file coordination
        return try byAccessor(url)
    }
