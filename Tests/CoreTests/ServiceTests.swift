...
final class ServiceTests: XCTestCase {
    ...
    func testServiceRegistration() async throws {
        ...
        XCTAssertTrue(resolvedService as AnyObject === service as AnyObject, "Resolved service should be the same instance that was registered")
    }
    ...
    func testServiceDependencyResolution() async throws {
        ...
        let dependentService = MockDependentService(container: container, dependencyId: MockService.serviceIdentifier)
    ...
    func testErrorHandling() async throws {
        ...
        do {
            ...
            XCTAssertTrue(error.localizedDescription.lowercased().contains("service not found"), "Error should mention service not found")
        }
        ...
        do {
            ...
            XCTAssertTrue(error.localizedDescription.lowercased().contains("dependency not ready"), "Error should mention dependency not ready")
        }
    }
}
...
actor MockService: ServiceTypes.UmbraService {
    ...
    func getSomething() async throws -> String {
        guard _state == .ready else {
            throw NSError(domain: "ServiceError", code: 100, userInfo: [NSLocalizedDescriptionKey: "Service not ready"])
        }
        return "Something"
    }
}
...
actor MockDependentService: ServiceTypes.UmbraService {
    ...
    func validate() async throws -> Bool {
        ...
        guard let container else {
            throw NSError(domain: "ServiceError", code: 100, userInfo: [NSLocalizedDescriptionKey: "Container not available"])
        }

        let dependentOn = try await container.resolveService(dependencyId)
        if await (dependentOn as? MockService)?.state != .ready {
            throw NSError(domain: "ServiceError", code: 101, userInfo: [NSLocalizedDescriptionKey: "Dependency not ready"])
        }
...
actor MockServiceContainer: ServiceContainer {
    ...
    func resolveService(_ identifier: String) async throws -> any ServiceTypes.UmbraService {
        guard let service = services[identifier] as? any ServiceTypes.UmbraService else {
            throw NSError(domain: "ServiceError", code: 102, userInfo: [NSLocalizedDescriptionKey: "The requested service not found"])
        }
        return service
    }
