...
enum TestUtilities {
    ...
    static func verifyDirectoryContent(
        source: String,
        destination: String,
        ignoreModificationTimes: Bool = true
    ) throws -> Bool {
        let fileManager = FileManager.default
        ...
        for relativePath in sourceContents {
            ...
            guard let sourceData = try? Data(contentsOf: URL(fileURLWithPath: sourcePath)),
                  let destData = try? Data(contentsOf: URL(fileURLWithPath: destPath)) else {
    ...
    private static func printDirectoryContents(_ path: String, contents: [String], indent: String) throws {
        for item in contents.sorted() {
            ...
            if isDirectory.boolValue {
                ...
                print("\(indent)📄 \(item) (\(ByteCountFormatter.string(fromByteCount: size, countStyle: .file)))")
            }
        }
    }
