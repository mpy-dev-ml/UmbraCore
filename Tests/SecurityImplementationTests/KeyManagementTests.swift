...
final class KeyManagementTests: XCTestCase {
    ...
    func testStoreAndRetrieveKey() async {
        // Create test key
        let testKey = SecureBytes(CryptoWrapper.generateRandomKey(size: 32))
    ...
    func testKeyRotation() async {
        ...
        let initialKey = SecureBytes(CryptoWrapper.generateRandomKey(size: 32))
        let testData = SecureBytes(Array("Sensitive data to be encrypted".utf8))
        ...
        let encryptionResult = Result { try CryptoWrapper.encryptAES_GCM(data: testData, key: initialKey, iv: iv) }
        ...
        guard let reencryptedData = reencryptedData else {
        ...
        do {
            ...
            let decryptResult = Result { try CryptoWrapper.decryptAES_GCM(data: newCiphertext, key: newKey, iv: newIv) }
