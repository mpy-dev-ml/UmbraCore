...
func analyzeModules(config: Config) -> [SwiftModule] {
  ...
  for moduleDir in moduleDirectories {
    ...
    if
      fileManager.fileExists(atPath: modulePath, isDirectory: &isDirectory) && isDirectory
        .boolValue
    {
  ...
  while let filePath=enumerator.nextObject() as? String {
    if filePath.hasSuffix(".swift") {
      ...
      do {
        ...
        let importMatches=importRegex.matches(
          in: content,
        ...
        let typeAliasMatches=typeAliasRegex.matches(
          in: content,
          range: NSRange(content.startIndex..., in: content)
        )
      ...
      for file in module.files {
        ...
        if
          let matches=regex?.matches(
            in: file.content,
            range: NSRange(file.content.startIndex..., in: file.content)
          )
        {
          updatedAliases[aliasIndex].usageCount += matches.count
          if
            matches.count > 0 && !updatedAliases[aliasIndex].usageLocations
...
func createRefactoringStepForIsolationPattern(module: SwiftModule) -> RefactoringStep {
  ...
  for isolationFile in module.isolationFiles {
    let isolationName=URL(fileURLWithPath: isolationFile.filePath).deletingPathExtension()
      .lastPathComponent
...
func generateAnalysisReport(modules: [SwiftModule], config: Config) {
  ...
  try? summaryReport.write(
    toFile: "\(config.outputDir)/module_analysis.md",
    atomically: true,
    encoding: .utf8
  )
      ...
      for (stepIndex, step) in plan.refactoringSteps.enumerated() {
        ...
        if !step.fileChanges.isEmpty {
          ...
          for change in step.fileChanges {
            let changeType=switch change.changeType {
              case .create: "Create"
              case .modify: "Modify"
              case .delete: "Delete"
              case .rename: "Rename"
  ...
  try? planReport.write(
    toFile: "\(config.outputDir)/refactoring_plan.md",
    atomically: true,
    encoding: .utf8
  )
...
func main() {
  ...
  let allTypeAliases=modules.flatMap(\.typeAliases)
  _=findTypeUsage(modules: modules, typeAliases: allTypeAliases)
