...
private final class ExampleMockXPCService: XPCServiceProtocolComplete {
    ...
    func sign(_ data: SecureBytes, keyIdentifier: String) async -> Result<SecureBytes, XPCSecurityError> {
        // Mock signature - just use random bytes
        return await generateRandomData(length: 32)
    }
    
    func verify(signature: SecureBytes, for data: SecureBytes, keyIdentifier: String) async -> Result<Bool, XPCSecurityError> {
    ...
    func getDiagnosticInfo() async -> Result<String, XPCSecurityError> {
        return .success("Mock XPC Service - All systems operational")
    }
