...
@unchecked Sendable {
  ...
  public func synchroniseKeys(_ syncData: SecureBytes) async throws {
    do {
      ...
      throw mapKeychainErrorToErrorHandlingDomains.UmbraErrors.Security.Protocols(
        error,
              ...
              reply: { [self] error in
                if let error {
                  let mappedError=(error as? InternalKeychainXPCError).map {
                    self.mapKeychainErrorToErrorHandlingDomains.UmbraErrors.Security.Protocols(
                      $0,
                      operation: "store"
                    )
                  } ?? ErrorHandlingDomains.UmbraErrors.Security.Protocols
  ...
  private func mapKeychainErrorToErrorHandlingDomains.UmbraErrors.Security.Protocols(
