...
/// Converts between ErrorHandlingDomains.UmbraErrors.Security.Protocols and
/// ErrorHandlingDomains.UmbraErrors.Security.ProtocolsDTO
public enum XPCSecurityDTOConverter {
  // MARK: - Convert to DTO

  /// Convert an ErrorHandlingDomains.UmbraErrors.Security.Protocols to
  /// ErrorHandlingDomains.UmbraErrors.Security.ProtocolsDTO
    ...
    switch error {
      case let .invalidKey(reason):
        return ErrorHandlingDomains.UmbraErrors.Security.ProtocolsDTO
          .invalidInput(details: "Invalid key: \(reason)")

      case let .invalidContext(reason):
        return ErrorHandlingDomains.UmbraErrors.Security.ProtocolsDTO
          .invalidInput(details: "Invalid context: \(reason)")

      case let .invalidParameter(name, reason):
        return ErrorHandlingDomains.UmbraErrors.Security.ProtocolsDTO
          .invalidInput(details: "Invalid parameter \(name): \(reason)")
      ...
      case let .unsupportedAlgorithm(name):
        return ErrorHandlingDomains.UmbraErrors.Security.ProtocolsDTO
          .unsupportedOperation(operation: "Algorithm: \(name)")

      case let .missingImplementation(component):
        return ErrorHandlingDomains.UmbraErrors.Security.ProtocolsDTO
          .unsupportedOperation(operation: component)
      ...
      @unknown default:
        return ErrorHandlingDomains.UmbraErrors.Security.ProtocolsDTO
          .unknown(details: "Unknown security error")
    }
  }

  // MARK: - Convert from DTO

  /// Convert an ErrorHandlingDomains.UmbraErrors.Security.ProtocolsDTO to
  ...
  public static func fromDTO(
    _ dto: ErrorHandlingDomains.UmbraErrors.Security
      .ProtocolsDTO
